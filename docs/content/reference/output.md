...
describes what users get. Pick a format with `-o`, or let ssi choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ssi <command> -o table   # aligned columns for reading
ssi <command> -o jsonl   # one JSON object per line, for piping
ssi <command> -o json    # a single JSON array
ssi <command> -o csv     # spreadsheet friendly
ssi <command> -o tsv     # tab-separated
ssi <command> -o url     # just the URL column
ssi <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ssi <command> --fields id,title,url
```
...
```bash
ssi <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ssi <command>            # a table, because this is a terminal
ssi <command> | wc -l    # JSONL, because this is a pipe
```
